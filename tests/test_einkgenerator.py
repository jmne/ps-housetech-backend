...
class TestEInkGenerator:
    ...
    def test_color_quantization(self, generator, rgb_array):
        ...
        assert all(pixel_color in ['BLACK', 'WHITE', 'RED'] for pixel_color in result)

    @pytest.mark.parametrize(
        'rgb_array, rgb_color, output',
        [
            ('tests/resources/get_layer_input.npy', 'WHITE', 'tests/resources/get_layer_output_WHITE.txt'),
            ('tests/resources/get_layer_input.npy', 'RED', 'tests/resources/get_layer_output_RED.txt'),
        ],
    )
    ...
    @pytest.mark.parametrize(
        'png, output',
        [('tests/resources/get_fused_layers_input_png.png', 'tests/resources/get_fused_layers_output.txt')],
    )
